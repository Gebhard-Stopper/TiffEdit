...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWndEx)
	...
	ON_COMMAND(ID_VIEW_HISTOGRAM, &CMainFrame::OnViewHistogramWindow)
	ON_UPDATE_COMMAND_UI(ID_VIEW_HISTOGRAM, &CMainFrame::OnUpdateViewHistogramWindow)
	ON_WM_SETTINGCHANGE()
	ON_COMMAND(ID_BTN_FILTER_THRESHOLD, &CMainFrame::OnBtnFilterThreshold)
	ON_COMMAND(ID_BTN_FILTER_OPENING, &CMainFrame::OnBtnFilterOpening)
	ON_COMMAND(ID_BTN_FILTER_CLOSING, &CMainFrame::OnBtnFilterClosing)
	ON_COMMAND(ID_BTN_FILTER_EROSION, &CMainFrame::OnBtnFilterErosion)
	ON_COMMAND(ID_BTN_FILTER_DILATION, &CMainFrame::OnBtnFilterDilation)
	ON_COMMAND(ID_BTN_FILTER_GAUSSIAN, &CMainFrame::OnBtnFilterGaussian)
	ON_COMMAND(ID_BTN_FILTER_ANISOTROPIC, &CMainFrame::OnBtnFilterAnisotropic)
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CLog::AppendLogWriter(new CScreenLogWriter(&m_wndOutput));
	CLog::InitWriters();	
...
BOOL CMainFrame::CreateDockingWindows()
{
	...
	CString strHistogramWnd;
	bNameValid = strHistogramWnd.LoadString(IDS_HISTOGRAM_WND);
	ASSERT(bNameValid);
	if (!m_wndHistogram.Create(strHistogramWnd, this, CRect(0, 0, 200, 200), TRUE, ID_VIEW_HISTOGRAM, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI))
	{
		TRACE0("Failed to create Histogram window\n");
		return FALSE; // failed to create
	}
...
void CMainFrame::SetDockingWindowIcons(BOOL bHiColorIcons)
{
	...
	//ToDO Set Icon for Histogram Window (ToDO: Create Icon ;-) )
...
void CMainFrame::OnViewHistogramWindow()
{
	m_wndHistogram.ShowPane(TRUE, FALSE, TRUE);
	m_wndHistogram.SetFocus();
}

void CMainFrame::OnUpdateViewHistogramWindow(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}
...
CTiffEditView* CMainFrame::GetActiveView()
{
	auto pFrame = GetActiveFrame();
	return static_cast<CTiffEditView*>(pFrame->GetActiveView());
}
